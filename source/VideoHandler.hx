...
class VideoHandler {
	...
	public static function playSmart(tag:String, file:String) {
		#if ios
		var mp4 = Paths.video(file + ".mp4");
		if (!sys.FileSystem.exists(mp4)) {
			trace("Missing iOS fallback video: " + mp4);
			return;
		}

		var player = new backend.VideoPlayerIOS(mp4, function() {
			if (PlayState.instance != null)
				PlayState.instance.callOnLuas('onVideoFinished', [tag]);
		});

		PlayState.instance.add(player.video);
		return;
		#end

		// Default WebM path for desktop + android
		playWebm(tag, file);
	}
